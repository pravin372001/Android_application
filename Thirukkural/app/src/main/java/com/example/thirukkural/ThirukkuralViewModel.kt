...
class ThirukkuralViewModel(private val context: Context) : ViewModel(), TextToSpeech.OnInitListener{
    private lateinit var kuralList: MutableList<String>
    private lateinit var _kural : MutableLiveData<String>
    val kural : LiveData<String>
        get() = _kural

    private lateinit var _kural1 : MutableLiveData<String>
    val kural1 : LiveData<String>
        get() = _kural1
    private lateinit var _kural2 : MutableLiveData<String>
    val kural2 : LiveData<String>
        get() = _kural2
    private lateinit var tts: TextToSpeech
    init{
        setKural()
        _kural = MutableLiveData<String>()
        _kural1 = MutableLiveData<String>()
        _kural2 = MutableLiveData<String>()
        initializeTTS()
    }
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            // Set language for TTS
            val result = tts.setLanguage(Locale("ta"))
            if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                Log.e("ThirukkuralViewModel", "Language not supported")
            }
        } else {
            Log.e("ThirukkuralViewModel", "Initialization failed")
        }
    }
    ...
    private  fun speakKural(kural: String?) {
        tts.speak(kural, TextToSpeech.QUEUE_FLUSH, null, null)
    }

    fun settKural(index: Int){
        Log.i("ThirukkuralViewModel", "index = $index")
